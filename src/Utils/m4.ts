import { cross, normalize, subtractVectors } from '../vector';
...
const m4 = {
  ...
  rotation: function(x:number, y:number, z:number){
    return m4.zRotate(
      m4.xRotate(
      m4.yRotation(y),x),z);
  },
  ...
  rotate:function(m:number[], x:number, y:number, z:number){
    return m4.zRotate(m4.xRotate(m4.yRotate(m,y),x),z);
  },
  ...
  lookAt:(cameraPosition:number[], target:number[], up:number[], dst=Array.from(new Array(16), n=>0))=>{
  ...
  inverseFloat:(m:Float32Array, dst=new Float32Array(16))=>{
